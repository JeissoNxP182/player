...
// JXP OPT 4: delegación de eventos para reducir listeners por tarjeta.
...
// JXP OPT 3: portadas con caché de URL + prioridad controlada para primeras tarjetas.
...
function toggleFavorite(event, item, buttonElement = null) {
  ...
  const boton = buttonElement || event?.currentTarget;

  if (
    boton &&
    boton.nodeType === 1 &&
    boton.classList?.contains('fav-btn')
  ) {
...
const POSTER_URL_CACHE = new WeakMap();

function getPosterUrl(item) {
  if (!item || typeof item !== 'object') return '';

  const cached = POSTER_URL_CACHE.get(item);
  ...
  POSTER_URL_CACHE.set(item, url);
  return url;
}

function preparePosterImage(img, item, isPriority = false) {
...
function createCardElement(item, cardIndex = 0) {
  const formattedDur =
  ...
  card.className = 'card';
  card.dataset.itemId = String(getItemId(item) || '');
  card.dataset.cardAction = 'open';
  ...
  favBtn.dataset.cardAction = 'favorite';
  favBtn.onclick = event => toggleFavorite(event, item, favBtn);
  ...
  if (currentSectionKey === 'historial') {
    ...
    historyBtn.dataset.cardAction = 'history-remove';
...
/* =========================================================
   ...
   - Un solo listener para las tarjetas, favoritos, historial y grupos.
   - Evita crear cientos de listeners cuando el catálogo crece.
   ========================================================= */

let CATALOG_EVENTS_BOUND = false;
...
function findCatalogItemById(itemId) {
  ...
  const sections = [
    ...
    LOCAL_CATALOG.peliculas,
    LOCAL_CATALOG.sagas,
    LOCAL_CATALOG.series,
    LOCAL_CATALOG.anime,
    LOCAL_CATALOG.videos,
    getFavorites(),
    getHistory()
  ];
  ...
  for (const data of sections) {
    ...
    for (const entry of data) {
      ...
      const directId = String(getItemId(entry) || '');
      if (directId === String(itemId)) return entry;

      if (Array.isArray(entry.peliculas)) {
        for (const peli of entry.peliculas) {
          if (!peli || visited.has(peli)) continue;
          visited.add(peli);
          if (String(getItemId(peli) || '') === String(itemId)) return peli;
        }
      }

      if (Array.isArray(entry.temporadas)) {
        for (const temp of entry.temporadas) {
          if (!Array.isArray(temp?.episodios)) continue;
          for (const ep of temp.episodios) {
            if (ep && String(getItemId(ep) || '') === String(itemId)) return ep;
          }
        }
      }
    }
...
function bindCatalogEvents() {
  if (CATALOG_EVENTS_BOUND) return;

  const container = $('catalogContainer');
  if (!container) return;

  CATALOG_EVENTS_BOUND = true;
  ...
  container.addEventListener('click', event => {
    ...
    if (action === 'saga-toggle') {
      event.preventDefault();
      const section = actionElement.closest('.saga-section');
    ...
    if (action === 'history-remove') {
      ...
      const item = findCatalogItemById(card?.dataset.itemId);
      if (item) removeFromHistory(event, item);
      return;
    }
...
function renderCatalog(items) {
  ...
  if (esEstructuraGrupos) {
    ...
    items.forEach(grupo => {
      ...
      sagaSection.className =
        'saga-section';
      sagaSection.dataset.cardAction = 'saga-section';
      ...
      sagaHeader.className =
        'saga-header';
      sagaHeader.dataset.cardAction = 'saga-toggle';
      ...
      grupo.peliculas.forEach(peli => {
        fragment.appendChild(
          createCardElement(peli, grid.children.length + fragment.childNodes.length)
        );
      });
  ...
  /* ---------- CATÁLOGO SIMPLE ---------- */
    ...
    items.forEach(item => {
      fragment.appendChild(
        createCardElement(item, fragment.childNodes.length)
      );
    });
    ...
    if (currentSectionKey === 'historial') {
      ...
      clearHistoryBtn.style.cssText = `
        ...
        cursor: pointer;
      `;
      clearHistoryBtn.dataset.cardAction = 'history-clear';
