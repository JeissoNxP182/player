...
// JXP CACHE UPDATE: remote catalogs refresh in background when persistent cache exists.
...
async function cargarSeccionRemota(sectionKey, forceReload = false, renderResult = true) {
  ...
  // Si existe una copia reciente en el dispositivo, la usamos primero.
  // Así una recarga/cierre del navegador no obliga a volver a descargar todo.
  if (!forceReload && sectionKey !== 'tv') {
    const catalogoGuardado = await leerCatalogoPersistente(sectionKey);

    if (Array.isArray(catalogoGuardado) && catalogoGuardado.length) {
      LOCAL_CATALOG[sectionKey] = catalogoGuardado;
      liberarCatalogosInactivos(sectionKey);

      if (renderResult) {
        renderCatalog(catalogoGuardado);
        updateSocialMeta(catalogoGuardado);
      }

      // La caché se muestra inmediatamente, pero comprobamos en segundo
      // plano si el JSON remoto cambió. Así los cambios en GitHub se reflejan
      // sin tener que esperar a que expire el TTL ni borrar IndexedDB.
      if (!SECTION_LOAD_PROMISES[sectionKey]) {
        void cargarSeccionRemota(sectionKey, true, false).then(datosNuevos => {
          if (!Array.isArray(datosNuevos)) return;

          LOCAL_CATALOG[sectionKey] = datosNuevos;

          if (currentSectionKey === sectionKey && renderResult) {
            renderCatalog(datosNuevos);
            updateSocialMeta(datosNuevos);
          }
        });
      }
  ...
  const carga = (async () => {
    ...
    return data;
      ...
      if (renderResult) {
        ...
        console.warn(`Precarga de ${sectionKey} no disponible:`, error);
      }
      return null;
    } finally {
...
window.addEventListener(
  ...
  async () => {
    ...
    if (!targetId) {
      ...
      // La precarga comienza después de mostrar Inicio.
      // Se ejecuta en segundo plano y no bloquea la interfaz.
      programarPrecargaInicial();
      return;
    }
...
/*
  ...
  - ignora tildes y ñ -> n
  - ignora espacios, guiones y signos
  - permite comparar VHS / V-H-S / V H S
*/
...
/*
  Variantes para errores ortográficos habituales.
  No cambiamos el texto que ve el usuario; solo generamos formas
  adicionales para comparar internamente.
...
function getSearchVariants(value) {
  ...
  const addPairVariants = (fromChars, replacement) => {
    let variant = base;
  ...
  // B/V: bhs <-> vhs, vienes <-> bienes, etc.
  addPairVariants(['v'], 'b');
  addPairVariants(['b'], 'v');

  // C/S/Z: reconocer / reconoser / reconozer.
  addPairVariants(['c', 'z'], 's');
  addPairVariants(['s', 'z'], 'c');
  addPairVariants(['c', 's'], 'z');

  // C/K/Q: pequeñas variaciones fonéticas frecuentes.
  addPairVariants(['c', 'q'], 'k');
  addPairVariants(['k', 'q'], 'c');
...
/*
  Distancia Damerau-Levenshtein simplificada:
  además de letras cambiadas/agregadas/eliminadas,
  tolera dos letras intercambiadas.
...
function fuzzyTitleMatch(title, query) {
  ...
  if (!titleVariants.length || !queryVariants.length) {
    return 0;
  }
  ...
  for (const t of titleVariants) {
    for (const q of queryVariants) {
      ...
      /*
        Para consultas cortas somos estrictos:
        1 error máximo para 1-3 letras.
        Así no aparecen resultados absurdos.
      */
      const maxDistance =
        q.length <= 3
          ? 1
      ...
      // También comparamos palabras completas.
      const words = t
        .split(/\s+/)
        .filter(Boolean);
      ...
      for (const word of words) {
        if (
          Math.abs(word.length - q.length) <=
          maxDistance
        ) {
          best = Math.min(
            best,
            damerauDistance(word, q)
          );
        }
      }
    }
  ...
  /*
    Coincidencia exacta por variante.
  */
  if (best === 0) {
    return 900;
  }
  ...
  /*
    La tolerancia aumenta con la longitud de la búsqueda.
    Esto permite:
      VHS -> BHS
      reconocer -> reconoser / reconozer
    pero evita resultados demasiado lejanos.
  */
  const longestQuery = Math.max(
    ...queryVariants.map(q => q.length)
  );
...
function fuzzySearchFieldMatch(text, query) {
  ...
  const textCompact =
    normalizeSearchCompact(text);

  const queryCompact =
    normalizeSearchCompact(query);

  if (!textCompact || !queryCompact) return 0;

  if (textCompact === queryCompact) {
    return 1000;
  }
  ...
  return fuzzyTitleMatch(text, query);
}
...
function getSearchScore(item, query, groupTitle = '') {
  ...
  score = Math.max(
    score,
    fuzzyTitleMatch(item.titulo, query)
  );

  if (tituloCompact === qCompact) {
    score = Math.max(score, 1000);
  } else if (tituloCompact.startsWith(qCompact)) {
    score = Math.max(score, 800);
  } else if (tituloCompact.includes(qCompact)) {
...
function programarPrecargaInicial() {
  const iniciar = async () => {
    // Si el usuario ya salió de Inicio, no seguimos gastando datos.
    if (currentSectionKey !== 'inicio') return;

    const secciones = ['peliculas', 'series', 'anime'];

    for (const sectionKey of secciones) {
      if (currentSectionKey !== 'inicio') break;
      ...
      if (necesitaCarga) {
        await cargarSeccionRemota(sectionKey, false, false);
      }
    }
...
async function ejecutarBusqueda(query, requestId) {
  ...
  if (!normalized) {
    if (currentSectionKey === 'inicio') {
      ...
      // Precarga inteligente: mientras el usuario está en Inicio,
      // preparamos algunos catálogos en segundo plano.
      // Se hace de forma escalonada para no saturar la conexión.
      programarPrecargaInicial();
    } else {
