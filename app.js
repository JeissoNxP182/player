...
async function cargarSeccionRemota(sectionKey, forceReload = false, renderResult = true) {
  ...
  // Caché persistente: mostramos inmediatamente la copia guardada para que
  // la página siga abriendo rápido, pero hacemos una actualización en segundo
  // plano para detectar cambios nuevos publicados en GitHub.
  if (!forceReload && sectionKey !== 'tv') {
    ...
    if (Array.isArray(catalogoGuardado) && catalogoGuardado.length) {
      ...
      // No bloqueamos la interfaz esperando la red. La petición fresca se
      // hace en segundo plano y reemplaza la caché cuando hay una versión nueva.
      if (!SECTION_LOAD_PROMISES[sectionKey]) {
        void cargarSeccionRemota(sectionKey, true, false).then(datosNuevos => {
          if (Array.isArray(datosNuevos) && datosNuevos.length) {
  ...
  const carga = (async () => {
    ...
    let respuesta;
    try {
      // Para catálogos remotos pedimos siempre la versión disponible en red.
      // Esto evita que el navegador conserve un JSON viejo después de publicar
      // cambios en GitHub Pages.
      const urlCatalogo = getRepositoryUrl(sectionKey);
      const separador = urlCatalogo.includes('?') ? '&' : '?';
      respuesta = await fetch(
        urlCatalogo + separador + '_jxp=' + Date.now(),
