...
function createCardElement(item, cardIndex = 0) {
  ...
  favBtn.onclick = event => {
    ...
    event.stopPropagation();
    toggleFavorite(event, item, favBtn);
  };
...
function bindCatalogEvents() {
  // Eventos directos en tarjetas y botones.
}
...
function renderCatalog(items) {
  ...
  if (esEstructuraGrupos) {
    ...
    items.forEach(grupo => {
      ...
      sagaHeader.onclick = event => {
        event.preventDefault();
        event.stopPropagation();

        const abierta =
          !sagaSection.classList.contains('collapsed');
        ...
        const arrow =
          sagaHeader.querySelector('.arrow-icon');
