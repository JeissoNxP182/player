...
const HISTORIAL_KEY = 'jxp_historial';
...
function fuzzyTitleMatch(title, query) {
  ...
  // Permite pequeños errores de escritura, por ejemplo:
  // VHS -> BHS. Para búsquedas muy cortas exigimos coincidencia
  // exacta o un solo carácter de diferencia.
  const maxDistance = q.length <= 3 ? 1 : q.length <= 6 ? 2 : 3;
  const candidates = [t, ...t.split(/\s+/)];

  let best = Infinity;
  for (const candidate of candidates) {
    if (Math.abs(candidate.length - q.length) > maxDistance) continue;
    best = Math.min(best, levenshteinDistance(candidate, q));
  }
...
async function prepararBusquedaGlobal() {
  ...
  // Cargamos solo los catálogos necesarios para buscar, pero dejamos
  ...
  const search = $('searchInput');
  const anterior = search?.placeholder || 'Buscar...';

  if (search) {
    search.placeholder = 'Cargando catálogo para buscar...';
...
async function filterCatalog() {
  ...
  // El buscador es GLOBAL desde cualquier sección. No es necesario
  // entrar primero en Películas, Sagas, Series, etc.
