...
function getSearchScore(item, query, groupTitle = '') {
  ...
  if (titulo === q) score = Math.max(score, 1000);
  else if (titulo.startsWith(q)) score = Math.max(score, 800);
  else if (titulo.includes(q)) score = Math.max(score, 600);
...
async function filterCatalog() {
  ...
  // Desde Inicio, la búsqueda es realmente global incluso
  // en la primera búsqueda después de abrir la página.
